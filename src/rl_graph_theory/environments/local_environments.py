...
class LocalSetEnvironment(GraphEnvironment):
    ...
    ``flattened_length`` edges (resp. arcs) are currently of the color 1; in the second
    ``flattened_length`` bits, the value 1 indicates which of the ``flattened_length`` edges (resp.
    arcs) are currently of the color 2; and so on until the ``(edge_colors - 1)``-th
    ...
    applied. The final ``graph_order`` bits from the state vector represent a one-hot encoding of
    the position that determines the vertex where the agent is currently located. In other words,
    there is exactly one value of 1 whose index determines the vertex where the agent is currently
    located.

    Each action is represented by a `numpy.int32` integer between 0 and
    ``edge_colors * graph_order - 1``. If the action number is ``a``, then the number
    ``a % graph_order`` signifies the vertex that the agent should move to from the current vertex,
    while ``a // graph_order`` determines which color the traversed edge (resp. arc) should be
    recolored with. If loops are not allowed, then the agent cannot move to a vertex where it is
    currently located.
    ...
    :ivar _episode_length: A positive `int` that determines the episode length of each of the
        episodes.
    :ivar _current_vertices: Either `None`, or a `numpy.ndarray` list of type `numpy.int32` that
    ...
    def __init__(
        ...
        :param starting_vertex: A nonnegative `int` between 0 and ``graph_order - 1`` that
...
class LocalFlipEnvironment(GraphEnvironment):
    ...
    ``flattened_length`` is the flattened length of the graphs to be constructed. In the state
    vector, in the first ``flattened_length`` bits, the value 1 indicates which of the
    ``flattened_length`` edges (resp. arcs) are currently of the color 1. Here, the edges (resp.
    arcs) are considered to be ordered in the row-major order or the clockwise order, as explained
    in the `FlattenedOrdering` enumeration. The user can select which of these two orderings should
    be applied. The final ``graph_order`` bits from the state vector represent a one-hot encoding
    of the position that determines the vertex where the agent is currently located. In other
    words, there is exactly one value of 1 whose index determines the vertex where the agent is
    currently located.
    ...
    represented by a `numpy.int32` integer between 0 and ``2 * graph_order - 1``. In this case, if
    the action number is ``a``, then the number ``a % graph_order`` signifies the vertex that the
    agent should move to from the current vertex, while ``a // graph_order`` is a binary value that
    determines whether the traversed edge (resp. arc) should be flipped. In other words, the number
    1 indicates that the proper edge color of the traversed edge (resp. arc) should be changed,
    while the number 0 indicates that the proper edge color of the traversed edge (resp. arc)
    should stay the same. On the other hand, if the ``flip_only`` parameter is set to `True`, then
    any traversed edge (resp. arc) must be flipped. In this case, each action is represented by a
    `numpy.int32` integer between 0 and ``graph_order - 1`` that gives the vertex that the agent
    should move to from the current vertex, with the traversed edge (resp. arc) being necessarily
    flipped. If loops are not allowed, then the agent cannot move to a vertex where it is currently
    located.
    ...
    :ivar _allow_loops: A `bool` that indicates whether the graphs to be constructed are allowed to
        have loops.
    :ivar _graph_order: A positive `int` that describes the order of the graphs to be constructed.
    :ivar _flip_only: A `bool` that indicates whether all the traversed edges (resp. arcs) should
    ...
    def __init__(
        ...
        :param episode_length: Either `None`, or a positive `int` that determines the episode
            length of each of the episodes. If the argument is `None`, then this means that the
            episode length should match the flattened length. The default value is `None`.
        :param flip_only: A `bool` that indicates whether all the traversed edges (resp. arcs)
