...
#     initial_graph=Graph.from_flattened(np.array([1, 0, 1, 1, 1, 0, 0, 1, 0, 0], dtype=np.uint8)),
#     edge_perturbation_probability=0.05,
# )

# print((generator(100).flattened_row_major ^ np.array([1, 0, 1, 1, 1, 0, 0, 1, 0, 0], dtype=np.uint8)).sum() / 1000)



generator = create_random_graph_generator(
    graph_order=6,
    edge_selection_probability=0.13,
    flattened_ordering=FlattenedOrdering.CLOCKWISE,
)
result = generator(100).flattened_row_major
print(result)
print(result.dtype)
print(result.sum() / 1500)