...
class GraphFormat(Enum):
    ...
    * ``format_representation`` is a `numpy.ndarray` that encodes the structure of the graph in the
    ...
    All formats with color numbers require $k \le 256$. The two bitmask formats require that the
    ...
    :cvar FLATTENED_ROW_MAJOR_BINARY: The flattened row-major format with binary slices. When this
        ...
        color numbers are equal to $0, 1, \\ldots, k - 1$, respectively. For fully colored graphs,
        the first row (corresponding to color 0) can be omitted; this variant is called the reduced
    ...
    :cvar FLATTENED_CLOCKWISE_COLORS: The flattened clockwise format with color numbers. When this
        format is used, the adjacency matrix entries are arranged in clockwise layer order:
        $(0, 0), (0, 1), (1, 1), (1, 0), (0, 2), (1, 2), (2, 2), (2, 1), (2, 0), \\ldots$. For
        directed graphs without loops, diagonal entries are skipped. For undirected graphs, only
        the upper-triangular entries are arranged, with or without the diagonal depending on
        whether loops are allowed. The arranged entries form a `numpy.ndarray` of type
        `numpy.uint8`; its length is called the flattened length.
